  ...
  <div class="viewport-container">
    <div class="game-screen">
      ...
      <div
        ...
        :class="['game-results-backdrop', { 'no-animation': gameResultsOverlayHasBeenShown }]"
      >
        <div :class="['game-results-wrapper', { 'no-animation': gameResultsOverlayHasBeenShown }]">
          <GameResults
...
.score-cards-container {
  ...
  bottom: calc(20px + 48px); /* Button top position: 20px (button bottom) + 48px (button height) */
  display: flex;
...
.modal-backdrop {
  ...
  background: linear-gradient(to right, transparent 0%, transparent 50%, rgba(2, 6, 23, 0.6) 50%, rgba(2, 6, 23, 0.6) 100%);
}
...
.modal-content {
  ...
  right: max(0px, calc(20px - env(safe-area-inset-right, 0px))); /* 20px from viewport edge, never negative */
  width: auto; /* Let left/right positioning determine width */
  max-width: calc(100vw - 20px); /* Ensure it doesn't exceed viewport minus 20px on right side */
  height: 100%;
  background-color: white;
  z-index: 1001;
...
.settings-modal-content {
  left: max(0px, calc(20px - env(safe-area-inset-left, 0px))); /* 20px from viewport edge, never negative */
  right: 0; /* Flush with viewport edge - content handles padding */
  width: auto; /* Let left/right positioning determine width */
  max-width: calc(100vw - 20px); /* Ensure it doesn't exceed viewport minus 20px on left side */
...
.settings-modal-backdrop {
  background: linear-gradient(to right, rgba(2, 6, 23, 0.6) 0%, rgba(2, 6, 23, 0.6) 50%, transparent 50%, transparent 100%);
}
...
.game-results-wrapper {
  width: calc(100% - 40px); /* Match content area width: game-screen width minus 20px padding on each side */
