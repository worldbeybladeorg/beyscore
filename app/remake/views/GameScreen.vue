<template>
  <div class="viewport-container">
    <!-- Orientation toggle - outside viewport -->
    <button class="orientation-toggle" @click="toggleOrientation" title="Switch to landscape">
    ...
    <div class="game-screen">
      ...
      <PlayerName
        v-if="(matchOver || (gameEnded && bestOf === null)) && winnerDisplayText"
      ...
      <div class="score-cards-container" :class="{ 'winner-displayed': (matchOver || (gameEnded && bestOf === null)) && winnerDisplayText }">
        <!-- Player 1 Score Card -->
        <ScoreCardPortrait 
          player="p1" 
          ...
          :bestOf="bestOf"
          :ownFinishEnabled="generation === 'x' && isOwnFinishEnabled"
          :score="player1Score.toString()"
          :disabled="gameEnded || matchOver"
          :filledStars="p1FilledStars"
          :showWarning="p1ShowWarning"
          :isFadingIn="isScoreFadingIn"
          :isShrinking="isScoreShrinking"
          @scoreIncrease="(points, chipLabel) => handlePlayer1ScoreIncrease(points, chipLabel)"
          @penalty="handlePlayer1Penalty"
          @warningToggle="handlePlayer1WarningToggle"
          @ownFinish="handlePlayer1OwnFinish"
        />
        ...
        <ScoreCardPortrait 
          ...
          :disabled="gameEnded || matchOver"
          :filledStars="p2FilledStars"
          :showWarning="p2ShowWarning"
          :isFadingIn="isScoreFadingIn"
          :isShrinking="isScoreShrinking"
          :class="{ 'winner-state': (matchOver || (gameEnded && bestOf === null)) && winnerDisplayText }"
          @scoreIncrease="(points, chipLabel) => handlePlayer2ScoreIncrease(points, chipLabel)"
          @penalty="handlePlayer2Penalty"
          @warningToggle="handlePlayer2WarningToggle"
          @ownFinish="handlePlayer2OwnFinish"
        />
        
        <!-- 20px gap spacer -->
        <div class="gap-to-buttons"></div>
      </div>

      <!-- Game Results Overlay -->
      <div
        v-if="showGameResultsOverlay"
        class="game-results-backdrop"
      >
        <div class="game-results-wrapper">
          <GameResults
            :winnerName="winnerNameForResults"
            :format="generation"
            :summaryItems="gameSummaryItemsForResults"
            :showOpponentSummary="generation === 'x' && (opponentOwnFinishCount > 0 || opponentPenaltyCount > 0)"
            :opponentOwnFinishCount="opponentOwnFinishCount"
            :opponentPenaltyCount="opponentPenaltyCount"
            @newGame="handleGameResultsNewGame"
            @viewHistory="handleGameResultsViewHistory"
      ...
      <div class="bottom-buttons-container">
        <MenuButton variant="matchHistory" :disabled="undoDisabled" @click="openMatchHistoryModal" />
        <UndoRedoButtons 
      ...
      <!-- Match History Modal -->
      <div v-if="isMatchHistoryModalOpen || isMatchHistoryModalClosing" class="modal-backdrop" :class="{ 'closing': isMatchHistoryModalClosing }" @click="closeMatchHistoryModal">
        <div class="modal-content" :class="{ 'closing': isMatchHistoryModalClosing }" @click.stop>
            ...
            <template v-for="(item, index) in matchHistory" :key="index">
              <div v-if="item.isGameDivider" class="game-divider">
                ...
                <div class="divider-text">
                  <span v-if="item.isMatchConclusion">Set {{ item.gameNumber }} – {{ getPlayerDisplayNameFull(item.winner) }} Wins</span>
              ...
              <MatchHistoryItem 
                ...
                :isGameWin="item.isGameWin || false"
                :isPenaltyWin="item.isPenaltyWin || false"
                :bestOf="bestOf"
                :setWins="item.setWins || 0"
      ...
      <!-- Settings Modal -->
      <div v-if="isSettingsModalOpen || isSettingsModalClosing" class="modal-backdrop settings-modal-backdrop" :class="{ 'closing': isSettingsModalClosing }" @click="closeSettingsModal">
        <div class="modal-content settings-modal-content" :class="{ 'closing': isSettingsModalClosing }" @click.stop>
          ...
          <div class="settings-content">
            <div
              v-if="gameHasStarted"
              class="settings-field alert-top"
            >
              <Alert />
            </div>
            ...
            <div class="settings-field generation-gap">
              <div style="position: relative;">
                <TextDropdownField
                  ...
                  variant="dropdown"
                  :showChevron="true"
                  :modelValue="beybladeGenerationLabel"
                  @toggle="handleGenerationDropdownToggle"
                />
                <div v-if="openDropdown === 'generation'" style="margin-top: 8px; position: absolute; width: 100%; z-index: 10;">
                  <DropdownMenu
            ...
            <div class="settings-field">
              <div style="position: relative;">
                <TextDropdownField
                  ...
                  variant="dropdown"
                  :showChevron="true"
                  :modelValue="pointsToWinLabel"
                  @toggle="handlePointsToWinDropdownToggle"
                />
                <div v-if="openDropdown === 'pointsToWin'" style="position: absolute; top: calc(0.875rem * 1.5 + 8px); width: 100%; z-index: 10; transform: translateY(calc(-100% - 8px));">
                  <DropdownMenu
                    :items="pointsToWinItems"
                    :selectedValue="matchTypeParam"
            ...
            <div v-if="matchTypeParam !== 'nolimit'" class="settings-field sets-gap">
              <div style="position: relative;">
                <TextDropdownField
                  ...
                  variant="dropdown"
                  :showChevron="true"
                  :modelValue="setsLabel"
                  @toggle="handleSetsDropdownToggle"
                />
                <div v-if="openDropdown === 'sets'" style="position: absolute; top: calc(0.875rem * 1.5 + 8px); width: 100%; z-index: 10; transform: translateY(calc(-100% - 8px));">
                  <DropdownMenu
                    :items="setsItems"
                    :selectedValue="bestOf ? bestOf.toString() : null"
            ...
            >
              ...
              <div class="own-finish-toggle">
                <ToggleButton
                  size="small"
                  :initialState="isOwnFinishEnabled"
          ...
          <div class="settings-buttons">
            <Button variant="blue" :disabled="!hasChanges" @click="handleSaveChanges">Save Changes</Button>
            <Button variant="secondary" @click="handleResetGame">Reset Game</Button>
          </div>
        </div>
      </div>
    </div>
...
const toggleOrientation = () => {
  ...
  if (currentPath === '/app/game') {
    // Switch to landscape
    router.push({ path: '/app/game/landscape', query })
  } else if (currentPath === '/app/game/landscape') {
    // Switch to portrait
    router.push({ path: '/app/game', query })
  }
}

// Get generation from route query, default to 'x' if not provided
const generation = computed(() => {
  const gen = route.query.generation
  if (gen && ['x', 'burst', 'mfb-zero-g', 'plastics-hms'].includes(gen)) {
    return gen
  }
  return 'x' // Default to X format
})

// Own Finish flag (from game setup, X format only)
const ownFinishEnabled = computed(() => {
  const flag = route.query.ownFinish
  return flag === '1' || flag === 'true' || flag === 'on'
})

// Get bestOf from route query, convert to number if provided
const bestOf = computed(() => {
  const bestOfValue = route.query.bestOf
  if (bestOfValue === '3' || bestOfValue === '5') {
    return parseInt(bestOfValue, 10)
  }
  return null // No stars if not provided
})
...
// Player names always default to Player 1/Player 2 on page refresh
...
const player1DisplayName = computed(() => formatDisplayName(player1NameSetting.value))
const player2DisplayName = computed(() => formatDisplayName(player2NameSetting.value))
...
const hasChanges = computed(() => {
  return player1NameSetting.value !== originalPlayer1Name.value ||
...
const gameSettingsChanged = computed(() => {
  return generation.value !== originalGeneration.value ||
         matchTypeParam.value !== originalMatchType.value ||
         bestOf.value !== originalBestOf.value ||
         isOwnFinishEnabled.value !== originalOwnFinish.value
})
...
// Get match type from route query
const matchTypeParam = computed(() => {
  const matchType = route.query.matchType
  if (typeof matchType === 'string' && matchType.length > 0) {
    return matchType
  }
  return generation.value === 'x' ? '4pts' : '3pts'
})

// Own Finish state
const isOwnFinishEnabled = ref(ownFinishEnabled.value)
watch(ownFinishEnabled, (newVal) => {
  isOwnFinishEnabled.value = newVal
})

const ownFinishOptionLabel = computed(() => (isOwnFinishEnabled.value ? 'On' : 'Off'))
...
const handleGenerationSelect = (value) => {
  // Update generation via route
  const newQuery = { ...route.query, generation: value }
  // If switching away from X, remove ownFinish if it exists
  if (value !== 'x' && newQuery.ownFinish) {
    delete newQuery.ownFinish
  }
  // Always default to base points to win for the selected generation
  // X format: 4pts, all others: 3pts
  if (value === 'x') {
    newQuery.matchType = '4pts'
  } else {
    newQuery.matchType = '3pts'
  }
  // If bestOf exists and we're switching to a format that doesn't support sets with the new base, remove it
  // (Actually, all formats support sets, so we don't need to remove bestOf)
  router.push({ path: route.path, query: newQuery })
  openDropdown.value = null
}

const handlePointsToWinSelect = (value) => {
  // Update match type via route
  const newQuery = { ...route.query, matchType: value }
  // If selecting No Limit, remove bestOf if it exists
  if (value === 'nolimit' && newQuery.bestOf) {
    delete newQuery.bestOf
  }
  router.push({ path: route.path, query: newQuery })
  openDropdown.value = null
}
...
const handleSetsSelect = (value) => {
  // Update bestOf via route
  const newQuery = { ...route.query }
  if (value === null) {
    // Remove bestOf if "None" is selected
    delete newQuery.bestOf
  } else {
    newQuery.bestOf = value
  }
  router.push({ path: route.path, query: newQuery })
  openDropdown.value = null
}
...
const p1SetWins = ref([]) // Array of booleans indicating which sets player 1 won
const p2SetWins = ref([]) // Array of booleans indicating which sets player 2 won
...
const displayedSetNumber = ref(1) // Track the displayed set number separately
...
const matchHistory = ref([]) // Array of { player: 'p1'|'p2', score1: number, score2: number, chipLabel: string, isWarning?: boolean, isPenalty?: boolean, isGameWin?: boolean, isGameDivider?: boolean, gameNumber?: number }
const currentGameNumber = ref(1) // Track current game number for sets
...
const closeSettingsModal = () => {
  ...
  // Revert route changes for game settings (generation, matchType, bestOf)
  // Note: These are updated immediately when dropdowns change, so we need to revert them
  const newQuery = { ...route.query }
  let routeChanged = false
  
  if (generation.value !== originalGeneration.value) {
    newQuery.generation = originalGeneration.value
    routeChanged = true
  }
  if (matchTypeParam.value !== originalMatchType.value) {
    newQuery.matchType = originalMatchType.value
    routeChanged = true
  }
  if (bestOf.value !== originalBestOf.value) {
    if (originalBestOf.value === null) {
      delete newQuery.bestOf
    } else {
      newQuery.bestOf = originalBestOf.value.toString()
    }
    routeChanged = true
  }
  
  // Revert Own Finish in route
  if (isOwnFinishEnabled.value !== originalOwnFinish.value) {
    if (originalOwnFinish.value && originalGeneration.value === 'x') {
      newQuery.ownFinish = 'true'
    } else {
      delete newQuery.ownFinish
    }
    routeChanged = true
...
const handleOwnFinishToggle = (state) => {
  isOwnFinishEnabled.value = state
}
...
const handleSaveChanges = () => {
  ...
  // Update route query with any changes (Own Finish only - names don't persist in route)
  const newQuery = { ...route.query }
  
  // Remove player names from route query so they reset on refresh
  delete newQuery.player1Name
  delete newQuery.player2Name
  
  // Update Own Finish in route (only if generation is X)
  if (generation.value === 'x') {
    if (isOwnFinishEnabled.value) {
      newQuery.ownFinish = 'true'
    } else {
      delete newQuery.ownFinish
    }
  } else {
    // If generation is not X, remove ownFinish from query
    delete newQuery.ownFinish
  }
  
  // Update route if there are changes
...
const historyIndex = ref(0) // Current position in history
...
const gameEnded = computed(() => {
  ...
  return player1Score.value >= maxPoints.value || player2Score.value >= maxPoints.value
})
...
const matchOver = computed(() => {
  ...
  const setsNeeded = Math.floor(bestOf.value / 2) + 1
  return p1SetWins.value.filter(won => won).length >= setsNeeded ||
...
const gameSummaryItemsForResults = computed(() => {
  ...
  const isMetalOrPlastics = generation.value === 'mfb-zero-g' || generation.value === 'plastics-hms'
...
watch(gameEnded, async (ended) => {
  if (ended && !pendingGameReset.value) {
    ...
    const actualWinningChip = winningChipLabel.value ? getChipTypeForHistory(winningChipLabel.value) : getPrimaryWinCondition()
    ...
    const isPenaltyWin = !winningChipLabel.value && // No win condition chip was used
                         mostRecentEntry && 
                         (mostRecentEntry.isPenalty === true) && 
                         !mostRecentEntry.isGameWin && // Not already a game win entry
                         ((p1Won && mostRecentEntry.player === 'p2') || (p2Won && mostRecentEntry.player === 'p1'))
...
const handlePlayer1ScoreIncrease = (points, chipLabel) => {
  ...
  if (scoreChanged) {
    ...
    if (chipLabel && !gameWillEnd) {
      ...
      const p1SetWinsCount = bestOf.value !== null ? p1SetWins.value.filter(won => won).length : 0
      matchHistory.value.unshift({
        player: 'p1',
...
const handlePlayer2ScoreIncrease = (points, chipLabel) => {
  ...
  if (scoreChanged) {
    ...
    if (chipLabel && !gameWillEnd) {
      ...
      const p2SetWinsCount = bestOf.value !== null ? p2SetWins.value.filter(won => won).length : 0
      matchHistory.value.unshift({
        player: 'p2',
...
const handleUndo = () => {
  if (historyIndex.value > 0) {
    ...
    const previousState = historyIndex.value > 0 ? history.value[historyIndex.value - 1] : null
    
    // Check if current state is a reset state (0-0 scores after a game ended)
    const isResetState = currentState.player1 === 0 && currentState.player2 === 0 && 
                        (currentState.p1SetWins?.length > 0 || currentState.p2SetWins?.length > 0)
    ...
    const isMatchConclusionState = previousState && 
                                   currentState.player1 === previousState.player1 &&
                                   currentState.player2 === previousState.player2 &&
                                   ((currentState.p1SetWins?.length || 0) !== (previousState.p1SetWins?.length || 0) ||
                                    (currentState.p2SetWins?.length || 0) !== (previousState.p2SetWins?.length || 0))
    ...
    const isDuplicateGameEndState = previousState && 
                                    currentState.player1 === previousState.player1 &&
                                    currentState.player2 === previousState.player2 &&
                                    ((currentState.p1SetWins?.length || 0) === (previousState.p1SetWins?.length || 0) &&
                                     (currentState.p2SetWins?.length || 0) === (previousState.p2SetWins?.length || 0))
...
.orientation-toggle {
  ...
  border: none;
  box-shadow: 0 2px 8px rgba(3, 7, 18, 0.05);
  display: flex;
  align-items: center;
  justify-content: center;
  cursor: pointer;
...
.score-cards-container {
  ...
  bottom: 68px; /* Buttons top at 68px (20px bottom + 48px height) - gap-spacer will add 20px */
  display: flex;
...
.score-cards-container :deep(.score-card) {
  width: 100%;
  max-width: 390px;
  height: calc((100% - 40px) / 2); /* Available height minus gap between cards (20px) and gap spacer (20px), divided by 2 */
  aspect-ratio: unset;
  flex-shrink: 0;
}
...
.set-indicator {
  position: absolute;
  /* Position at the center of the gap between the two cards */
  /* Top of score-cards-container + height of first card + half of 20px gap - half of indicator height (19px) */
  /* Container top: var(--status-bar-height) + 20px */
  /* Container available height: 100% (game-screen) - var(--status-bar-height) - 20px - 68px */
  /* First card height: (container available height - 40px) / 2 */
  /* Gap center: 10px (half of 20px gap) */
  /* Adjust for indicator center: -19px (half of 38px height) */
  top: calc(var(--status-bar-height) + 20px + ((100% - var(--status-bar-height) - 20px - 68px - 40px) / 2) + 10px - 19px);
  left: 50%;
...
.winner-message {
  position: absolute;
  /* Position at the center of the gap between the two cards - same as set-indicator */
  /* Top of score-cards-container + height of first card + half of 20px gap - half of indicator height (19px) */
  /* Container top: var(--status-bar-height) + 20px */
  /* Container available height: 100% (game-screen) - var(--status-bar-height) - 20px - 68px */
  /* First card height: (container available height - 40px) / 2 */
...
.bottom-buttons-container {
  position: absolute;
  bottom: 20px; /* 20px from bottom of content area (safe area padding already applied by game-screen) */
  left: calc(20px + var(--safe-area-inset-left));
  right: calc(20px + var(--safe-area-inset-right));
  display: flex;
  flex-direction: row;
...
.match-history-list {
  position: absolute;
  top: calc(20px + 1.6875rem + 24px); /* Modal padding (20px) + title height (1.125rem * 1.5) + 24px gap */
  left: 20px;
  right: 20px;
...
.settings-content {
  position: absolute;
  top: calc(20px + 1.6875rem + 6px + 1.5rem + 24px); /* Title + description spacing + description height + 24px */
  left: 20px;
  right: 20px;
  bottom: 0;
  width: calc(100% - 40px);
...
.settings-buttons :deep(.custom-button) {
  flex: 1;
  width: 100%;
  font-weight: bold;
}
