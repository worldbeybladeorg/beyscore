// From https://github.com/elk-zone/elk/blob/a5ec0cbd3f14123318295225f200f31eab7ed263/scripts/generate-pwa-icons.ts
...
const publicFolders = ["public"].map((folder) => resolve(root, folder));
    ...
    await Promise.all(
      icons.ico.sizes.map(async (size) => {
        const png = await sharp(
          resolve(
            folder,
            icons.iconName("transparent", size).replace(/-temp\.png$/, ".png"),
          ),
        )
          .toFormat("png")
          .toBuffer();
        await writeFile(
          resolve(folder, icoName(size)),
          new Uint8Array(ico.encode([png])),
        );
      }),
    );
  }
...
console.log("Generating BeyScore PWA Icons...");
...
generatePWAIcons(publicFolders, <Icons>{
  ...
  .then(() => console.log("Beyscore PWA Icons generated"))
  .catch(console.error);
